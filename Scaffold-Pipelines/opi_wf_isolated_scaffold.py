...
for _,mol in df.iterrows():
    ...
    smi = mol["Smiles"]
    sub = mol["Sub_Type"]
    structure = get_struct_from_smiles(smiles=smi)
    ...
    calculator_opt = get_opt_calculator(
        basename=f"{keys}_opt",
        structure=structure,
        cores=args.cores,
        working_dir=calc_space_name,
    )
    output_opt, opt_struct = run_opt_calc(calc=calculator_opt)
    opt_struct, iodine_index = align_ci_xaxis(opt_struct, return_i_index=True)
    opt_struct.smiles = smi
    opt_struct.charge = q
    opt_struct.multiplicity = mult
    calculator_sp = get_sp_calculator(
        basename=f"{keys}_sp",
        structure=opt_struct,
        cores=args.cores,
        working_dir=calc_space_name,
    )
    output_sp = run_sp_calc(calc=calculator_sp)
    prop_file = calculator_sp.working_dir / f"{calculator_sp.basename}.property.json"
    with open(prop_file, "r") as f:
        prop_data = json.load(f)
    el_prop = get_el_prop_from_prop_file(prop_data)

    # Energies TODO Pull E_tot from final SP E not E_SCF = Done, need to check if this works
    E_tot = float(prop_data["Geometries"][0]["Single_Point_Data"]["FinalEnergy"])

    gbw_basename = f"{keys}_sp"
    subprocess.run(
    ...
    mbis_prop = get_mbis_from_prop_file(output_sp.property_json_data)
    ...
    mwfn_input = create_mwfn_input(
        ...
        elements=get_elem_list(opt_struct),
        mbis=True,
        becke=False,
    )
    run_mwfn(mwfn_input, molden_main)
    mw = MwfnFuzzySpaceOut(calculator_sp.working_dir / "mwfn_mbis.out")
    dump_mwfn_fuzzy_results(mw)

    # Create ASE Atom Object
    symbols = [atom.element for atom in opt_struct.atoms]
    coords_angstrom = [
        [atom.element, atom.coordinates.x, atom.coordinates.y, atom.coordinates.z]
        for atom in opt_struct.atoms
    ]
    positions = np.array([[x, y, z] for _, x, y, z in coords_angstrom])
    atoms = Atoms(symbols=symbols, positions=positions)

    # Add properties from ORCA
    atoms.energy = E_tot  # Total energy in Hartree
    atoms.info["ORCA_mol_dipole_sp_au"] = (
        el_prop["molecular_dipole"].reshape(3).tolist()
    )
    atoms.info["ORCA_atomic_dipole"] = (
        el_prop["atomic_dipole"].tolist()
    )
    atoms.info["ORCA_mol_quadrupole_sp_au"] = (
        el_prop["molecular_quadrupole"].reshape(6).tolist()
    )

    atomic_quad_array = el_prop["atomic_quadrupole_array"]  # np.array (natoms, 6)
    MBIS_quad_array = el_prop["mbis_atomic_quadrupoles_array"]
    ...
    atoms.info["ORCA_iso_quadrupole_au"] = float(el_prop["isotropic_quadrupole"])
    atoms.info["ORCA_MBIS_Atomic_Charges"] = el_prop["mbis_atomic_charges"].tolist()
    atoms.info["ORCA_MBIS_r3"] = el_prop["mbis_3rd_radial_moment"].tolist()
    atoms.info["ORCA_MBIS_Atomic_Dipoles"] = (el_prop["mbis_atomic_dipoles"].tolist())
    atoms.info["ORCA_MBIS_Atomic_Quadrupoles"] = np.array(MBIS_spherical_quads).tolist()
    atoms.info["ORCA_MBIS_Atomic_Octupoles"] = (el_prop["mbis_atomic_octupoles_array"].tolist())
    atoms.info["ORCA_mol_polarizability_iso"] = float(el_prop["molecular_polarizability_iso"])
    atoms.info["ORCA_atomic_polarizability_iso"] = el_prop["atomic_polarizability_iso"].tolist()
