...
methodology as ROBERT:
  - Features : I_monopole, I_dipole_0, I_C6, I_r2, ring_charge
               (ROBERT selection + iodine descriptors + scaffold; single Cl acceptor)
  ...
  - Split    : 20% even/stratified test set (evenly distributed across y range)
...
# ══════════════════════════════════════════════════════════════════════════════
# Target DB config  (simplified: Cl-molecules.db only, no acceptor parameters)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# Step 2 — assemble dataset (only Cl-molecules.db)
# ══════════════════════════════════════════════════════════════════════════════

records = []

try:
    tdb = connect(TARGET_DB_FILENAME)
except Exception as e:
    raise RuntimeError(f"Could not open '{TARGET_DB_FILENAME}': {e}")
...
for row in tdb.select():
    ...
    records.append(dict(
        name          = f"{TARGET_LABEL}-{scaffold}-{mol_name}",
        target        = TARGET_LABEL,
        scaffold      = scaffold,
...
print(f"  {TARGET_DB_FILENAME}: {n_added} entries")
...
# ══════════════════════════════════════════════════════════════════════════════
# Step 3 — ROBERT-style even train/test split (20% test)
# Even split: sort by y, then interleave to get uniform coverage of y range
# ══════════════════════════════════════════════════════════════════════════════

def even_train_test_split(X, y, test_size=0.2, seed=0):
    """
    ROBERT 'even' split: sorts by y-value and picks every n-th sample for test
    to ensure even coverage of the response range.
    """
    rng      = np.random.default_rng(seed)
    n        = len(y)
    n_test   = max(4, int(np.round(n * test_size)))
    sorted_idx = np.argsort(y)

    # Stride-sample from sorted indices for even y-distribution in test set
    stride   = n // n_test
    test_idx = sorted_idx[::stride][:n_test]

    # Shuffle any remainder into train
...
X_train, X_test, y_train, y_test, idx_train, idx_test = even_train_test_split(
    X_all, y_all, test_size=0.2, seed=SEED
)

print(f"Train: {len(y_train)} | Test: {len(y_test)}")

# ══════════════════════════════════════════════════════════════════════════════
# Step 4 — StandardScaler (fit on train only, as ROBERT does)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# Step 5 — Bayesian Optimisation (ROBERT methodology)
...
for tgt in all_targets:
    ...
    if mask.any():
        ax.scatter(y_train[mask], y_pred_train[mask],
                   facecolors='grey',
                   linewidths=1.2, s=50, zorder=2, label='Training set')
...
ax.set_title('GB Model — Residual Plot', fontsize=12)
sh, sl = ax.get_legend_handles_labels()
...
for ax, (resid, split_label) in zip(
    axes,
    [(resid_train, "Training Residuals"), (resid_test, "Test Residuals")]
):
...
def predict(I_monopole: float, I_dipole_0: float, I_C6: float, I_r2: float,
    ...
    X = scaler.transform([[I_monopole, I_dipole_0, I_C6, I_r2, ring_charge]])
